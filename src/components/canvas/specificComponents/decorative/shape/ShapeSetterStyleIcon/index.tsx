...
export function ShapeSetterStyleIcon(props: any) {
    const setterType = useSelector((state: RootState) => state.canvas.setterType);
    const dispatch = useDispatch();
    const displaySetter = (e: any) => {
        e.stopPropagation();
        dispatch(setSetterType('shape'));
    }
    return <img src={setterType === 'shape' ? Selected : Unselected} style={{display: 'block'}} onMouseDown={displaySetter}/>
}