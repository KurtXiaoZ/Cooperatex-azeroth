...
import { removeComponent, setComponent, setComponentHeight, setComponentPosition, setComponentStyle, setComponentWidth } from 'redux/pagesReducer';
import store from 'redux/store';
import { renderSetterIcons } from '../../specificComponents';
import Draggable, {DraggableData} from 'react-draggable';
const shiftedDownKeys: any = [];
const shiftedUpKeys: any = [];
let shiftedDistanceUp = Number.MAX_VALUE;
 ...
 */
export function SwitcherComponent(props: any) {
    ...
    useEffect(() => {
        if(!store.getState().pages[props.pageKey]?.components[props.componentKey]?.fixed) {
            for(let curKey in components) {
                const curComponent = JSON.parse(JSON.stringify(components[curKey]));
                if(curComponent.type === 'menu' || curKey === props.componentKey) continue;
                const middleY = curComponent.y + Math.floor(parseInt(curComponent.style.height) / 2);
                if(middleY > component.y) {
                    shiftedDownKeys.push(curKey);
                    curComponent.y += parseInt(component.style.height);
                    dispatch(setComponent({pageKey: props.pageKey, componentKey: curKey, component: curComponent}));
                }
                else {
                    shiftedUpKeys.push(curKey);
                    if(Math.abs(component.y - (curComponent.y + parseInt(curComponent.style.height))) < shiftedDistanceUp) shiftedDistanceUp = Math.abs(component.y - (curComponent.y + parseInt(curComponent.style.height)));
                }
            }
            shiftedUpKeys.forEach((upKey: string) => {
                const upComponent = JSON.parse(JSON.stringify(components[upKey]));
                upComponent.y -= shiftedDistanceUp;
                dispatch(setComponent({pageKey: props.pageKey, componentKey: upKey, component: upComponent}));
            });
        }
        return function() {
            if(!store.getState().pages[props.pageKey]?.components[props.componentKey]?.fixed) {
                shiftedDownKeys.forEach((downKey: string) => {
                    console.log(111);
                    const downComponent = JSON.parse(JSON.stringify(components[downKey]));
                    dispatch(setComponent({pageKey: props.pageKey, componentKey: downKey, component: downComponent}));
                });
                shiftedUpKeys.forEach((upKey: string) => {
                    const upComponent = JSON.parse(JSON.stringify(components[upKey]));
                    dispatch(setComponent({pageKey: props.pageKey, componentKey: upKey, component: upComponent}));
                });
                if(setterType !== 'switcherManage') {
                    dispatch(removeComponent({pageKey: props.pageKey, componentKey: props.componentKey}));
                }
            }
        }
    }, []);
    ...
    const resize = (e: any, data: DraggableData, type: string) => {
        if(type === 'bottomleft' || type === 'bottomright') {
            shiftedDownKeys.forEach(async (downKey: string) => {
                const downComponent = JSON.parse(JSON.stringify(components[downKey]));
                downComponent.y += (data.deltaY);
                await dispatch(setComponent({pageKey: props.pageKey, componentKey: downKey, component: downComponent}));
            });
        }
        else {
            shiftedUpKeys.forEach(async (upKey: string) => {
                const upComponent = JSON.parse(JSON.stringify(components[upKey]));
                upComponent.y += (data.deltaY);
                await dispatch(setComponent({pageKey: props.pageKey, componentKey: upKey, component: upComponent}));
            });
        }
    ...
    return <>
        ...
        <Draggable
            bounds={{left: component.x + 10, top: 0, right: canvas.offsetWidth, bottom: component.y + parseInt(component.style.height) - 10}}
            axis={axis}
            grid={[1,1]}
            offsetParent={canvas}
        ...
        <Draggable
            bounds={{left: 0, top: component.y + 10, right: component.x + parseInt(component.style.width) - 10, bottom: canvas.offsetHeight - 6}}
            axis={axis}
            grid={[1,1]}
            offsetParent={canvas}
        ...
        <Draggable
            bounds={{left: component.x + 10, top: component.y + 10, right: canvas.offsetWidth, bottom: canvas.offsetHeight - 6}}
            axis={axis}
            grid={[1,1]}
            offsetParent={canvas}
        ...
        <div className='canvas-component-setters' 
            style={{
                ...
                top: component.y + parseInt(component.style.height) + 'px'
            }}
        >
